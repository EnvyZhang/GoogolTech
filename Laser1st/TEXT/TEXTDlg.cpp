...
kk
{
	...
	{
		rtn = GT_LnXYEx(CardNum,1,0,20000,10,1,0,0);								 //压入第一段位置指令 
		rtn = GT_LnXYEx(CardNum,1,20000,20000,10,1,0,0);                           //压入第二段位置指令 
		rtn = GT_LnXYEx(CardNum,1,20000,0,10,1,0,0);								 //压入第三段位置指令  
		rtn = GT_LnXYEx(CardNum,1,0,0,10,1,0,0);									//压入第四段位置指令
		rtn = GT_LnXYEx(CardNum,1,40000,40000,10,1,0,0);							//压入第四段位置指令
		rtn = GT_BufLaserOffEx(CardNum,1,0,LaserChanel);                          //关闭激光
		rtn = GT_CrdDataEx(CardNum,1, NULL, 0);									//将前瞻缓存区中的数据压入控制器
	}
...
void CTEXTDlg::OnBnClickedButton3()
{
	...
	//rtn = GT_BufLaserFollowOff(CardNum,1,0,LaserChanel);//停止能量跟随
...
void CTEXTDlg::OnBnClickedChecknewahead()
{
	...
	axisLimitMode[0] = AXIS_LIMIT_MAX_VEL | AXIS_LIMIT_MAX_ACC | AXIS_LIMIT_MAX_DV;  //限制轴1、2的速度、加速度和跳变速度
	axisLimitMode[1] = AXIS_LIMIT_MAX_VEL | AXIS_LIMIT_MAX_ACC | AXIS_LIMIT_MAX_DV;
	
	lookAheadPara.lookAheadNum = 200;    //前瞻段数，建议设置200段
	lookAheadPara.time = 0.01;          //时间常数
	lookAheadPara.radiusRatio = 1;      //曲率限制调节参数
	for (i=0;i<8;++i)
	{
	...
	rtn = GT_SetAxisFollowModeLa(CardNum,1,axisFollowMode);//设置跟随模式
	rtn = GT_SetAxisLimitModeLa(CardNum, 1, axisLimitMode); //设置轴限制模式
	rtn = GT_SetAxisVelValidModeLa(CardNum,1,0xF);//设置轴速度有效，按位设置，0xF表示前4个轴
	//rtn = GT_CrdClear(CardNum,1,0);//即将把数据存入坐标系的 FIFO0 中，所以要首先清除此缓存区中的数据
	rtn = GT_InitLookAheadEx(CardNum,1,&lookAheadPara,0);//设置前瞻参数（需要放在最后设置）
	if(rtn != 0)
	{
